...
char audit_trail_sw=DEFAULT_AUDIT_SW, load_desc_sw; 
...
/*        diskette_ready();     */               /* make sure ready      */
	...
	if(load_sw == OLD)                      /* is it an OLD load    */
	{
      ...
      printf("\nIF YOU ONLY WANT TO CHANGE THE DESCRIPTION, TYPE < D >");
		printf("\nTO REPLACE ENTIRE PACKAGE, HIT < ENTER >:");
		read_kb(&ch, 1);
		if (toupper(ch) == 'D') 
          load_desc_sw = 'D'; 	 /* Load description only */
		else
			 load_desc_sw = 'N'; 
...
/* delete all files in the sinn directory       */
	if (load_desc_sw == 'D')      /* delete description and describe.txt */
{
	sprintf(buffer, "%s\\*.D*", sin_dir);            /* make CSCS\CSCSSINn\*.* */
	printf("removing old description files so that new files can be loaded, please wait\n");
	...
	sprintf(buffer, "%s\\DESCRIBE.TXT", sin_dir);
	unlink(buffer);
	fcopy(describe,     sin_dir);           /* copy DESCRIBE.TXT    */
   sprintf(sys_buf, "call deschop %s %s %-04.4d %d %d %d", 
...

{
	...
	printf("\nremoving old files so that new files can be loaded, please wait\n");
	if(0 == (_dos_findfirst(buffer, _A_NORMAL, &buf_dir)))    /* any file */
	{
		sprintf(buffer, "%s\\%s", sin_dir, buf_dir.name);
		unlink(buffer);
